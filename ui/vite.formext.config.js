...
/**
 * Separate build for the form-extension bundle (the Cucumber Test panel on the work item form). Intentionally NOT part of the SPA build: the server-rendered fragment
 * (webapp/cucumber/html/cucumber-panel.html, emitted by CucumberFormExtension) imports this
 * by a fixed URL and calls its named export (`mountCucumberPanel`), so it must be a stable-name ES
 ...
 *
 * Output: dist/app/assets/cucumberPanel.js, appended next to the SPA bundle (emptyOutDir: false),
 * so the SPA build (which empties dist/app) must run first. Served under
 * /polarion/cucumber-app/ui/app/assets/.
 */
export default defineConfig({
  ...
  build: {
    ...
    lib: {
      ...
      entry: {
        cucumberPanel: fileURLToPath(new URL('./src/formext/mount.tsx', import.meta.url)),
      },
      formats: ['es'],
