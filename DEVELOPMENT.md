...
│   │       ├── META-INF/
│   │       ├── layout/     # Form-extension fragment injected into the work item form
│   │       └── webapp/     # Web resources (cucumber = REST, cucumber-app = the React bundles)
│   └── test/               # Test sources
...
The `ui/` Vitest suite runs in the `test` phase alongside the Java tests. By default it runs the full
suite (behavior + visual regression) inside the pinned Playwright Docker image so the screenshots match
the committed references. These `-D` flags adjust that:
